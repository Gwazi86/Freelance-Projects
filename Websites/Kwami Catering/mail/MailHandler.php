<?php
	$stripHTML = true;
	$owner_email = "info@kwamicatering.co.za";
	// $headers = 'From:' . $_POST["contact_email"];
	$subject = 'Subject: ' . $_POST["contact_subject"];
	$messageBody = "";
	
	if($_POST['fname']!='nope'){
		$messageBody .= '<p><strong>First Name: </strong>' . $_POST["Name"] . '</p>' . "\n";
		$messageBody .= '<br>' . "\n";
	}
	//if($_POST['lname']!='nope'){
	...
	if($_POST['Number']!='nope'){		
		$messageBody .= '<p><strong>Phone Number: </strong>' . $_POST['Telephone'] . '</p>' . "\n";
		$messageBody .= '<br>' . "\n";
	}
	...
	try{
		if(!mail($owner_email, $subject, $messageBody)){
			...
			header("Location: https://www.kwamicatering.co.za/");
			die();
